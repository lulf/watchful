...
pushd blinky && cargo objcopy --release -- -O binary ../blinky.bin && popd

#mergehex -m boot-recovery.hex app-recovery.hex -o infinitime-recovery.bin

imgtool create --align 4 --version 1.0.0 --header-size 32 --slot-size 475136 --pad-header blinky.bin blinky-image.bin
